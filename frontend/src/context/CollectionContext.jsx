...
export function CollectionContextProvider({ children }) {
  ...
  const fetchUserCollections = async (id) => {
    try {
      const response = await axios.get(`${import.meta.env.SITE_URL}/api/collections`, { params: { id }})
  ...
  const fetchCollectionById = async (id) => {
    try {
      const response = await axios.get(`${import.meta.env.SITE_URL}/api/collections/id/${id}`)
  ...
  const handleCollectionAdd = async (id, pinId) => {
    try {
      await axios.post(`${import.meta.env.SITE_URL}/api/collections/add`, { id, pinId })
  ...
  const handleCollectionRemove = async (id, pinId) => {
    try {
      await axios.post(`${import.meta.env.SITE_URL}/api/collections/remove`, { id, pinId })
      const updatedCollection = await fetchCollectionById(id)
      setSelectedCollection(updatedCollection)
    } catch(error) {
